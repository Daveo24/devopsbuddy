    ...
    public String signUpPost(@RequestParam(name = "planId") int planId,
        ...
        if (planId == PlansEnum.BASIC.getId()) {
            ...
            if (StringUtils.isEmpty(payload.getCardCode()) ||
                ...
                LOG.error("One or more credit card fields is null or empty. Returning error to user");
                model.addAttribute(SIGNED_UP_MESSAGE_KEY, false);
            ...
            Map<String, Object> customerPrams = new HashMap<>();
            customerPrams.put("description", "Username: " + payload.getUsername());
            customerPrams.put("email", payload.getEmail());
//            customerPrams.put("plan", selectedPlan.getId());

            LOG.info("Subscribing the customer to plan {}", selectedPlan.getName());
            String stripeCustomerId = stripeService.createCustomer(stripeTokenParams, customerPrams);
    ...
    public ModelAndView signupException(HttpServletRequest request, Exception exception) {
        ...
        LOG.error("Request {} rasied exception {}", request.getRequestURL(), exception);
